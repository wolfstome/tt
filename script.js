...
// 1. CSV URL: 🎉 FIXED! This uses the correct published CSV format to read data.
// Source: https://docs.google.com/spreadsheets/d/e/2PACX-1vRqqcKPAStgKwqTNo4HYEltpwLGgXlUa-eIfyF5X5L9ATJF4GS2yi43cVxjshWeaYPlOfGYI1gzs6Ci/pub?gid=0&single=true&output=csv
const WISHES_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRqqcKPAStgKwqTNo4HYEltpwLGgXlUa-eIfyF5X5L9ATJF4GS2yi43cVxjshWeaYPlOfGYI1gzs6Ci/pub?gid=0&single=true&output=csv'; 

// 2. WEB APP URL: Confirmed correct for writing/submitting the wishes.
...
            
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}. Check Sheet Publishing settings.`);
            }
            
            const csvData = await response.text();
            
            // The fixed URL should return CSV data directly.
            
...
            if (wishesFeed) {
                 wishesFeed.innerHTML = '<p class="text-center text-danger">Failed to load wishes. Please ensure the Google Sheet is published to the web. Reason: ' + error.message + '</p>';
            }
        }
    }
...
                wishForm.reset();
                
                // Wait 3 seconds for the Google Sheet to process, then refresh the displayed wishes 
